...
async function findAgentBin(name) {
  ...
  return new Promise((resolve) => {
    if (process.platform === 'win32') {
      ...
      exec('powershell.exe -NoProfile -Command "Get-Command -Name \'' + name + '\' -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source"', { 
        timeout: 8000, 
        windowsHide: true 
      }, (err, stdout) => {
...
async function releaseInspect(p) {
  ...
  // 跨平台检查 gh CLI
  if (process.platform === 'win32') {
    out.gh = !!(await sh('powershell.exe', ['-NoProfile', '-Command', 'Get-Command gh -ErrorAction SilentlyContinue | Select-Object -ExpandProperty Source']));
  } else {
...
async function claudeOAuthToken() {
  ...
  if (PLATFORM === 'darwin') {
    // macOS: 从 Keychain 获取 OAuth token
    try {
...
// curl 直连 api.anthropic.com 会被 403 地域拦截。此时读系统代理（macOS: scutil，Windows: 注册表）兜底。
async function curlSysProxyLine() {
  ...
  if (PLATFORM === 'darwin') {
    ...
    try {
      const out = await new Promise((resolve, reject) => {
        exec('powershell.exe -NoProfile -Command "$r=\'HKCU:\\Software\\Microsoft\\Windows\\CurrentVersion\\Internet Settings\'; $e=(Get-ItemProperty -Path $r -Name ProxyEnable -EA SilentlyContinue).ProxyEnable; if($e-eq 1){$s=(Get-ItemProperty -Path $r -Name ProxyServer -EA SilentlyContinue).ProxyServer; if($s){if($s-match\'^\'){$s} else {\'http://\'+$s}}}"', { 
          timeout: 3000, 
          windowsHide: true 
        }, (err, stdout) => (err ? reject(err) : resolve(stdout)));
      });
      const proxy = String(out || '').trim();
