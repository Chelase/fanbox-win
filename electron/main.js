...
function createWindow() {
  ...
  win.webContents.on('render-process-gone', (event, details) => {
    console.error('[renderer] 崩溃:', details);
  });
  
  // 捕获页面加载失败
  win.webContents.on('did-fail-load', (event, errorCode, errorDescription, validatedURL) => {
    console.error(`[renderer] 加载失败: ${errorDescription} (${validatedURL})`);
  });
...
function screenshotDir() {
  if (process.platform === 'darwin') {
    ...
    // Windows: 默认截屏位置是图片文件夹下的 Screenshots
...
function startShotWatch() {
  ...
  const screenshotPatterns = {
    darwin: /^(截屏|截圖|截图|Screenshot|Screen Shot|CleanShot|SCR-)/i,
    win32: /^(Screenshot|Screen Shot|Snip|截图)/i,
    linux: /^(Screenshot|Screen Shot|截图)/i
  };
  
  const pattern = screenshotPatterns[process.platform] || screenshotPatterns.linux;
  
  try {
    shotWatcher = fs.watch(dir, { persistent: false }, (evt, filename) => {
      const name = filename ? filename.toString() : '';
      // 截屏写盘有中间态，跳过；只认系统截屏的命名习惯
      if (!pattern.test(name) || !/\.(png|jpe?g)$/i.test(name)) return;
...
function trySetDisableSleep(on) {
  if (process.platform === 'darwin') {
    ...
    try {
      ...
      if (on) {
        // 设置系统执行状态：保持系统唤醒
        execSync('powershell.exe -NoProfile -Command "Add-Type -TypeDefinition \'using System; using System.Runtime.InteropServices; public class Sleep { [DllImport(\"kernel32.dll\")] public static extern uint SetThreadExecutionState(uint esFlags); }\'; [Sleep]::SetThreadExecutionState(0x80000001)"', { 
          windowsHide: true, 
          stdio: 'ignore' 
        });
      } else {
        // 恢复系统执行状态：允许休眠
        execSync('powershell.exe -NoProfile -Command "Add-Type -TypeDefinition \'using System; using System.Runtime.InteropServices; public class Sleep { [DllImport(\"kernel32.dll\")] public static extern uint SetThreadExecutionState(uint esFlags); }\'; [Sleep]::SetThreadExecutionState(0x80000000)"', { 
          windowsHide: true, 
          stdio: 'ignore' 
        });
      }
...
async function setLidIntent(on) {
  ...
  if (on) {
    ...
    const detail = process.platform === 'darwin'
      ...
      : M('开启后，只要还有终端会话在跑，系统不会自动休眠——agent 任务能接着干。\n\n注意：持续耗电发热，建议接电源。终端全部退出或退出翻箱时自动恢复正常休眠。',
          'While any terminal session is running, the system won\'t sleep — your agent tasks keep going.\n\nNote: it keeps drawing power and heat; stay plugged in. Normal sleep is restored once all terminals exit or you quit FanBox.');
